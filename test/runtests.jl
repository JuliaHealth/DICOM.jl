...
dcmMG = dcm_parse(joinpath(testdir, "DISCIMG/IMAGES/MGIMAGEA"))

@testset "Loading DICOM data" begin
    @test dcmMR[(0x0008,0x0060)].data == "MR"
    @test dcmCT[(0x0008,0x0060)].data == "CT"
    @test dcmMG[(0x0008,0x0060)].data == "MG"

    @test length(dcmMR[(0x7FE0,0x0010)].data) == 65536
    @test length(dcmCT[(0x7FE0,0x0010)].data) == 262144
    @test length(dcmMG[(0x7FE0,0x0010)].data) == 262144
...
@testset "Writing DICOM data" begin
    ...
    outIO = open(outMG1, "w+"); dcm_write(outIO,dcmMG); close(outIO)
end
...
@testset "Consistence of Reading/Writing data" begin
    ...
    outIO = open(outMG2, "w+"); dcm_write(outIO,dcmMG1); close(outIO)
    ...
    @test dcmMR1[(0x0008,0x0060)].data == "MR"
    @test dcmCT1[(0x0008,0x0060)].data == "CT"
    @test dcmMG1[(0x0008,0x0060)].data == "MG"

    @test length(dcmMR1[(0x7FE0,0x0010)].data) == 65536
    @test length(dcmCT1[(0x7FE0,0x0010)].data) == 262144
    @test length(dcmMG1[(0x7FE0,0x0010)].data) == 262144

    # Test lookup-by-fieldname; cross-compare dcmMR with dcmMR1
    @test dcmMR1[(0x0008,0x0060)].data == lookup(dcmMR, "Modality").data
    @test dcmMR1[(0x7FE0,0x0010)].data == lookup(dcmMR, "Pixel Data").data
end
